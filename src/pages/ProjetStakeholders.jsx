      ...
      {[1, 2, 3, 4, 5].map((n) => (
        <button
          ...
          style={{
            ...
            background: n <= value ? 'var(--color-text-primary)' : 'var(--color-bg-tertiary)',
            fontSize: 0,
          }}
          title={`${n}`}
        />
...
function EditCell({ value, onSave, multiline, placeholder, width }) {
    ...
    return (
      <div
        ...
        style={{
          ...
          fontSize: 12, color: value ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)', lineHeight: 1.4,
          width: width || '100%',
  ...
  const commonStyle = {
    width: '100%', padding: '4px 6px', borderRadius: 5, border: '1.5px solid var(--color-accent)',
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1100 }}>
          ...
          <tbody>
            {stakeholders.map((sh) => {
              ...
              return (
                <tr key={sh.id} style={{ borderBottom: '0.5px solid var(--color-border-soft)', verticalAlign: 'top' }}>
                  {/* Rôle */}
                  ...
                  <td style={{ padding: '10px 12px', minWidth: 110 }}>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)' }}>{sh.influence}/5</span>
                  </td>
                  {/* Intérêt */}
                  <td style={{ padding: '10px 12px', minWidth: 110 }}>
                    <ScoreSelect value={sh.interet} onChange={(v) => upd(sh.id, 'interet', v)} />
                    <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)' }}>{sh.interet}/5</span>
                  </td>
                  {/* Success definition */}
                  ...
                  <td style={{ padding: '8px 12px', minWidth: 130 }}>
                    <select
                      ...
                      style={{ fontSize: 12, padding: '4px 8px', borderRadius: 5, border: '1px solid var(--color-border)', outline: 'none', background: 'var(--color-bg-card)', cursor: 'pointer' }}
                    >
                  ...
                  <td style={{ padding: '8px 12px', minWidth: 140 }}>
                    <input
                      ...
                      style={{ fontSize: 12, padding: '4px 6px', borderRadius: 5, border: '1px solid var(--color-border)', outline: 'none' }}
                    />
                  </td>
                  {/* Actions */}
            ...
            {stakeholders.length === 0 && (
              <tr>
                <td colSpan={11} style={{ padding: 48, textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: 13 }}>
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)', fontSize: 11, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.08em', height: 120 }}>
            ← Influence →
          </div>
        </div>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 8, paddingLeft: 2, paddingRight: 2 }}>
            <div style={{ textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Intérêt faible</div>
            <div style={{ textAlign: 'center', fontSize: 11, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Intérêt élevé</div>
          </div>
              ...
              return (
                ...
                >
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--color-text-tertiary)' }}>{qm.desc}</p>
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      ...
                      >
                        ...
                        {tooltip === sh.id && sh.success_definition && (
                          <div style={{
                            ...
                            background: 'rgb(26, 26, 24)', color: '#FFFFFF', borderRadius: 8, padding: '8px 12px',
                    ...
                    {items.length === 0 && (
                      <span style={{ fontSize: 11, color: 'var(--color-text-tertiary)', fontStyle: 'italic' }}>Aucun stakeholder</span>
                    )}
                  </div>
                </div>
              );
      ...
      {/* Légende */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 24, padding: '16px', background: 'var(--color-bg-secondary)', borderRadius: 12 }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--color-text-tertiary)', width: '100%' }}>Légende</span>
        {Object.entries(QUADRANT_META).map(([key, qm]) => (
          <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}><strong style={{ color: qm.color }}>{qm.label}</strong> — {qm.desc}</span>
          </div>
        ))}
      </div>
    </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ display: 'flex', border: '1px solid var(--color-border)', borderRadius: 7, overflow: 'hidden' }}>
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', border: 'none', cursor: 'pointer', fontSize: 13, background: vue === 'tableau' ? 'var(--color-text-primary)' : 'var(--color-bg-card)', color: vue === 'tableau' ? 'var(--color-bg-primary)' : 'var(--color-text-secondary)' }}
              >
                <List size={14} /> Tableau
              </button>
              <button
                onClick={() => setVue('matrice')}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', border: 'none', cursor: 'pointer', fontSize: 13, background: vue === 'matrice' ? 'var(--color-text-primary)' : 'var(--color-bg-card)', color: vue === 'matrice' ? 'var(--color-bg-primary)' : 'var(--color-text-secondary)', borderLeft: '1px solid var(--color-border)' }}
              >

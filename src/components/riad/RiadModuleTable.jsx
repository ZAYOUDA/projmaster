...
const btnSecStyle = { padding: '8px 14px', borderRadius: 6, border: '1px solid var(--color-border)', background: 'var(--color-bg-card)', color: 'var(--color-text-primary)', fontSize: 13, fontWeight: 500, cursor: 'pointer' };
const iconBtnStyle = { padding: 5, borderRadius: 6, border: '1px solid var(--color-border)', background: 'var(--color-bg-card)', cursor: 'pointer', display: 'flex', color: 'var(--color-text-secondary)' };
const thStyle = { padding: '10px 12px', textAlign: 'left', fontSize: 12, fontWeight: 500, color: 'var(--color-text-tertiary)', whiteSpace: 'nowrap' };
const tdStyle = { padding: '10px 12px', fontSize: 12.5, color: 'var(--color-text-primary)', maxWidth: 220 };
    ...
    <div>
          ...
          {['tous', ...STATUS_LEVELS.map((s) => s.key)].map((s) => (
            <button
              ...
              style={{
                ...
                background: filter === s ? 'var(--color-text-primary)' : 'var(--color-bg-card)',
                color: filter === s ? 'var(--color-bg-primary)' : 'var(--color-text-secondary)',
              }}
            >
      ...
      <div style={{ background: 'var(--color-bg-card)', border: '0.5px solid var(--color-border)', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {items.map((it) => (
              <tr key={it.id} style={{ borderBottom: '0.5px solid var(--color-border-soft)' }}>
                <td style={{ ...tdStyle, fontFamily: 'monospace', color: 'var(--color-text-tertiary)' }}>
            ...
            {items.length === 0 && (
              <tr><td colSpan={listFields.length + 2} style={{ padding: 40, textAlign: 'center', color: 'var(--color-text-tertiary)' }}>Aucune entrée.</td></tr>
            )}
          </tbody>
        </table>
      </div>

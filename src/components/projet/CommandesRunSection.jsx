    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-secondary)' }}>Lignes (collaborateur × PU × NBJ commandé)</span>
        ...
        {lignes.length === 0 && <p style={{ fontSize: 12, color: 'var(--color-text-tertiary)', margin: 0 }}>Aucune ligne.</p>}
        {lignes.map((l) => (
          <div key={l.id} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
            ...
            />
            <button type="button" onClick={() => removeLigne(l.id)} style={{ ...iconBtnStyle, color: 'var(--color-danger)' }} title="Retirer">
              <Trash2 size={13} />
            </button>
          </div>
        ))}
...
export default function CommandesRunSection({ projet, collaborateurs }) {
  ...
  return (
    <section style={cardStyle}>
      ...
      {commandes.length === 0 && <p style={{ fontSize: 13, color: 'var(--color-text-tertiary)' }}>Aucune commande pour {anneeVue}.</p>}
      {commandes.map((cmd) => (
        <div key={cmd.id} style={{ border: '0.5px solid var(--color-border)', borderRadius: 8, padding: 12, marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>
              {cmd.numero}
              {cmd.lot ? <span style={{ color: 'var(--color-text-tertiary)', fontWeight: 400 }}> · {cmd.lot}</span> : null}
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => setEditing(cmd)} style={iconBtnStyle} title="Modifier"><Pencil size={13} /></button>
              <button onClick={() => handleDelete(cmd)} style={{ ...iconBtnStyle, color: 'var(--color-danger)' }} title="Supprimer"><Trash2 size={13} /></button>
            </div>
          </div>
